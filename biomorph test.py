...
class Node(object):
    ...
    def addNodePair(self, i, biomorph):
        ...
        for x in range(2):
            ...
            pygame.draw.line(win, Color_line, (node.pos), (newnode.pos))
            LorR *= -1
...
treenumber = 15
run = True  # loop to test different parameters
while(run):
    for x in range(15): # draws 15 trees
        ...
        x= (x+1)*(1000/treenumber) # set the inital starting point of the tree
        y= 300
        ...
        pygame.display.flip()
        biomorph.nodes.append(startnode)
        for i in range(1, abs(biomorph.genome[0])+1):  # recursion depth of 6
            for node in biomorph.nodes:
                if node.depth == i-1:
                    node.addNodePair(i, biomorph)#length,anglemagnitude)
